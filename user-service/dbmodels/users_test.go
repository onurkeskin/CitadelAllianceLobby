...
func testUserOneToOneUsersInfoUsingUsersInfo(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var foreign UsersInfo
	var local User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, usersInfoDBTypes, true, usersInfoColumnsWithDefault...); err != nil {
	...
	check, err := local.UsersInfo().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&local}
	if err = local.L.LoadUsersInfo(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UsersInfo == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.UsersInfo = nil
	if err = local.L.LoadUsersInfo(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UsersInfo == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpUsersInfoUsingUsersInfo(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, usersInfoDBTypes, false, strmangle.SetComplement(usersInfoPrimaryKeyColumns, usersInfoColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, usersInfoDBTypes, false, strmangle.SetComplement(usersInfoPrimaryKeyColumns, usersInfoColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*UsersInfo{&b, &c} {
		err = a.SetUsersInfo(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ConfirmationCode`: `character varying`, `Email`: `character varying`, `ID`: `uuid`, `ModifiedDate`: `date`, `Roles`: `ARRAYcharacter varying`, `Status`: `character varying`, `Username`: `character varying`}
	_           = bytes.MinRead
)
