...
var (
	userColumns               = []string{"username", "email", "status", "confirmation_code", "roles", "modified_date", "id"}
	userColumnsWithoutDefault = []string{"username", "email", "status", "confirmation_code", "roles", "modified_date", "id"}
...
func (o *User) UserPassword(mods ...qm.QueryMod) userPasswordQuery {
	...
	queries.SetFrom(query.Query, "\"users\".\"user_password\"")

	return query
}

// UsersInfo pointed to by the foreign key.
func (o *User) UsersInfo(mods ...qm.QueryMod) usersInfoQuery {
	queryMods := []qm.QueryMod{
		qm.Where("user_id=?", o.ID),
	}
	...
	query := UsersInfos(queryMods...)
	queries.SetFrom(query.Query, "\"users\".\"users_info\"")
...
func (userL) LoadUserPassword(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`users.user_password`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for user_password")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_password")
	}
...
// LoadUsersInfo allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (userL) LoadUsersInfo(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`users.users_info`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load UsersInfo")
	}

	var resultSlice []*UsersInfo
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice UsersInfo")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for users_info")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for users_info")
	}
	...
	if singular {
		foreign := resultSlice[0]
		object.R.UsersInfo = foreign
		if foreign.R == nil {
			foreign.R = &usersInfoR{}
		}
		foreign.R.User = object
	}
...
// SetUsersInfo of the user to the related item.
// Sets o.R.UsersInfo to related.
// Adds o to related.R.User.
func (o *User) SetUsersInfo(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UsersInfo) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.UsersInfo = related
	}

	if related.R == nil {
		related.R = &usersInfoR{
			User: o,
		}
	} else {
		related.R.User = o
